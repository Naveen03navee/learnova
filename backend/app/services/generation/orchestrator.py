...
async def _publish_event(session_id: UUID, status: GenerationStatus, message: str, progress: float, batch: int = None, total_batches: int = None):
...
async def process_generation_session(session_id: UUID):
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            await db.commit()
            await _publish_event(session_id, session.status, "Retrieving relevant knowledge...", 0.05)
            ...
            async def _run_batch(batch_idx, batch_count, is_supplementary=False):
                ...
                await _publish_event(session_id, session.status, f"Generating {batch_count} questions for {batch_label}...", 
                                     ...
                                     batch=batch_idx, total_batches=session.total_batches)
                ...
                while current_attempt < max_attempts and valid_in_this_batch < target_for_batch and total_llm_calls < settings.GENERATION_MAX_TOTAL_LLM_CALLS:
                    ...
                    try:
                        ...
                        ai_response = await ai_manager.generate(primary_provider, gen_request)
            ...
            await _publish_event(session_id, session.status, f"Generation finished. {session.valid_count} valid, {session.duplicate_count} duplicates, {session.invalid_count} invalid.", 1.0, batch=session.current_batch, total_batches=session.total_batches)
            ...
            await _publish_event(session_id, GenerationStatus.FAILED, f"Generation failed: {str(e)}", 1.0)
